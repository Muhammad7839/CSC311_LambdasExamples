...
public class ArraysAndStreams2 {
   public static void main(String[] args) {
      ...
      List<String> startsWithVowel = Arrays.stream(strings)
              .filter(s -> {
                 String first = s.toLowerCase();
                 return first.startsWith("a") || first.startsWith("e") ||
                         first.startsWith("i") || first.startsWith("o") ||
                         first.startsWith("u");
              })
              .collect(Collectors.toList());
      System.out.printf("Strings starting with a vowel: %s%n", startsWithVowel);

      // concatenate all strings with a comma
      String joined = String.join(", ", strings);
      System.out.printf("Concatenated string: %s%n", joined);

      // count of strings longer than 5 characters
